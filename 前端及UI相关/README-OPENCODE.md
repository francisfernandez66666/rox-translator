...
**如果你要照着切图还原界面，先读 `UI-ANNOTATIONS.md`**——它是 43 张切图的**文案真值表**（每段文字、字号、字重、颜色、放在哪个位置、用哪个组件），全部从设计源文件直接抽取。

**不要用 OCR 识别切图**：纯黑底 + 灰阶文字会让 OCR 大量错字、丢字。以 `UI-ANNOTATIONS.md` 为准，切图只用来核对版式。

另外记住一条硬规则：文档里出现的 emoji（`🌐 📊 💬 🔔 👤 🤖 🛠 ⎋` 等）**是画布里的图标占位，不是文案**，实现时换成 16×16 SVG 图标或直接删掉。

<!-- ============================================================================
   ...
   职责：系统健康状态卡片（KB/余额/流程/熔断/错误率）+ 最近审计日志 + 导出 CSV
    （审计区块与 CSV 导出仅超管可见——后端 /api/system/audit 已收紧为仅超管）
   ============================================================================ -->
<template>
  <section class="ad-section">
    <h2>{{ t('overview.title') }}</h2>
    <button class="ad-btn" @click="loadDash">{{ t('overview.refresh') }}</button>
    <!-- 审计日志接口仅超管可用：导出入口按角色显隐 -->
    <button v-if="isSuper" class="ad-btn ad-btn-green" style="margin-left: 8px" @click="exportAuditCSV">{{ t('overview.exportAuditCsv') }}</button>
    ...
    <!-- 最近审计（仅超管可见；审计接口已收紧为仅超管） -->
    <div v-if="isSuper && audit && audit.length" class="ad-audit">

...
- **行业字典（2026-09-10）**：超管在后台「知识库 → 🏭 行业管理」面板在线创建/维护行业（code 唯一、启停、删除带引用保护）；全站行业下拉动态拉取（注册/租户表单/数据采集），注册页走公开 register-config（仅启用行业）、新建/停用即时生效
- **RAG 术语遵循硬闸**：源文命中 KB L1 术语（如 极石→ROX）时，译文必须体现规定译法，否则硬闸拦截并附 KB 提示自动重译（默认 ≤8 次）
- **品牌名统一译法（2026-09-10）**：知识库可独立配置品牌名（module=brand，如 极石/极石汽车→ROX，前端「知识库→🏷️ 品牌名」面板可视化配置）；对话与文件翻译在**翻译前做品牌保护**（源文品牌名替换为规定译法，杜绝音译成 Киджиш）并**翻后归一化**（剥离 "ROX vehicles"/"Автомобили ROX" 等自创后缀），保证品牌名在外语一律等于规定译法
- **租户与组织隔离**：多租户、角色权限、部门预算
- **品牌定制与登录页布局**：按子域名解析租户品牌（名称/Logo/背景图）；登录页支持全屏背景或左右分栏
- **Token 计费**：双桶台账（发放额度 + 永久余额）、预充值、订阅与发票；实时计费余额不足即时中止并明确提示「请充值」；文件/文本工单建单前按源规模预检余额，不足直接拒绝避免中途失败
- **邀请裂变**：推荐注册、邀请奖励

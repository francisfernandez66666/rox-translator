...
| 前端 | React 18 + TDesign + React Router + Zustand（Vite 构建，React.lazy 代码分割） |
...
- **品牌官网首页**：未登录 `/` 为营销 Landing（价值主张/积分价目卡动态拉取/FAQ/信任条/CTA），UTM 五参捕获进注册归因；SEO（description/og/canonical/JSON-LD）；`/pricing` 公开页积分口径 + 首月半价
...
bash scripts/uat/run_uat.sh                    # 全链路 UAT 主矩阵（PostgreSQL 方言=生产同构，发布闸门：API A49+B67 + 功能/交易专项 308（含 T42 USDT 全链 mock_chain 驱动、T43/T44 修复回归、T45 密码找回全链路） + Playwright 33（mobile_uat/a11y 超时已根治，0 flaky）；首轮非零自动 --last-failed 复跑甄别 flaky）
bash scripts/uat/assist_uat.sh                 # AI 顾问 UAT（32 断言：C端链路/同义词/兜底改造/config 白名单与掩码/LLM 热加载/测试连通/CRUD/内嵌管理页/主库 Token 桥接与 env 优先级）
...
cd frontend-react && npx vitest run            # 前端单测（87 用例，含工单质检徽标与 AI 助手面板 jsdom 测试）

...
| 文档同步 | ✅ | 部署指南 §一/§二 更新（fileproc 两个辅助 py 与旧栈的区分已注明；前端目录已切为 frontend-react/）；PROGRESS.md 补架构决策记录 |
...
| 前端栈切换 | ✅ | Vue 前端已移除，React + TDesign 前端（frontend-react/）成为默认/规范前端 |
...
# 2) frontend-react/dist 不存在时提示先 npm run build（或带 -b 参数代跑）
# 3) 启动 Go 服务 127.0.0.1:8787（-frontend frontend-react/dist -kb data/tm_embeddings.npz -kbdb data/dev.db）
...
- 部署指南.md「本地构建」「本地开发环境」章节：删除 Python venv/pip 相关行，
  本地启动命令改指 start.sh；前端目录已切为 frontend-react/；依赖清单删 poppler 之外的 Python 段落说明保留（生产 PDF 管线仍需 venv：
...
2. `./start.sh` 一键拉起 Go + frontend-react，登录/翻译全链路可用；`grep -rn "python\|pip\|uvicorn" start.sh build.sh` 零命中。

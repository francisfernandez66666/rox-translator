...
func (s *Store) EnsureRefCode(uid int64) string {
	var code string
	var tid int64
	db.QueryRow(s.db, db.CurrentDialect(), "SELECT tenant_id, COALESCE(ref_code,'') FROM users WHERE id=?", uid).Scan(&tid, &code)
	if code != "" {
		return code
	}
	b := make([]byte, 4)
	...
	for {
		...
		err := db.QueryRow(s.db, db.CurrentDialect(), "SELECT ref_code FROM users WHERE ref_code=?", code).Scan(&exists)
	...
	db.Exec(s.db, db.CurrentDialect(), "UPDATE users SET ref_code=? WHERE id=? AND COALESCE(ref_code,'')=''", code, uid)
	return code
}
...
func (s *Store) GrantTrialStack(inviterUID, inviterTID, inviteeUID, tokens int64, days int) error {
	...
	res, err := db.Exec(s.db, db.CurrentDialect(), "INSERT INTO referral_rewards (inviter_uid,inviter_tid,invitee_uid,invitee_email,type,tokens,days,created_at) VALUES (?,?,?,?,?,?,?,?)",
	...
	err = db.QueryRow(s.db, db.CurrentDialect(), "SELECT MAX(expires_at) FROM quota_grants WHERE tenant_id=? AND kind IN ('trial','plan') AND expires_at>?",
...
func (s *Store) RewardPaidPermanent(inviteeUID, tokens, days int64) error {
	...
	if _, err := db.Exec(s.db, db.CurrentDialect(), "INSERT INTO referral_rewards (inviter_uid,inviter_tid,invitee_uid,invitee_email,type,tokens,days,created_at) VALUES (?,?,?,?,?,?,?,?)",
	...
	if days > 0 {
		...
		if gerr := s.CreateQuotaGrant(inviterTID, "referral_paid", tokens, expT, "invite", inviteeUID); gerr != nil {
			...
			_, _ = db.Exec(s.db, db.CurrentDialect(), "UPDATE balance_accounts SET balance=balance+? WHERE tenant_id=?", tokens, inviterTID)
		}
		return nil
	}
	// 永久奖励（默认）：直接累加永久余额
	_ = s.EnsureBalance(inviterTID)
	_, err = db.Exec(s.db, db.CurrentDialect(), "UPDATE balance_accounts SET balance=balance+? WHERE tenant_id=?", tokens, inviterTID)
	return err
}

...
type Alert struct {
	...
	UserID     int64  `json:"user_id"`     // 关联用户 ID（系统/租户级告警为 0）
	Level      string `json:"level"`       // 告警级别：info / warning / critical
	Kind       string `json:"kind"`        // 告警类型：balance / model / error_rate
	Message    string `json:"message"`     // 告警消息内容（简短摘要）
	Log        string `json:"log"`         // 详细日志/上下文（比 message 更完整）
...
// CreateAlert 记录告警（兼容旧调用，userID=0、log="")；同类型+租户的 open 告警幂等去重。
// 参数：tid=租户 ID，level=告警级别，kind=告警类型，message=告警描述。
func (s *Store) CreateAlert(tid int64, level, kind, message string) error {
	return s.CreateAlertEx(tid, level, kind, message, 0, "")
}

// CreateAlertEx 记录告警（扩展版）；同类型+租户的 open 告警不重复创建（幂等去重）。
// 参数：tid=租户 ID，level=告警级别，kind=告警类型，message=简短摘要，
//
...
func (s *Store) ListAlerts(tid int64, status string, limit int) ([]*Alert, error) {
	...
	query := "SELECT id, tenant_id, user_id, level, kind, message, log, status, created_at, COALESCE(resolved_at,'') FROM alerts"
	args := []interface{}{}
	conds := []string{}
	if tid > 0 {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&a.ID, &a.TenantID, &a.UserID, &a.Level, &a.Kind, &a.Message, &a.Log, &a.Status, &a.CreatedAt, &a.ResolvedAt); err != nil {

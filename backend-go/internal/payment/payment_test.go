...
// wechat 未配置资质应报错；资质不全（缺证书序列号/商户私钥）同样 fail-closed 拒绝出单；
// 回调验签（AES-256-GCM）独立可用。真实下单协议的完整正/反例见 gateway_sdk_test.go。
func TestWechatProvider(t *testing.T) {
	...
	// 只配了 AppID/商户号/APIv3 密钥，缺证书序列号与商户私钥：仍须拒绝出单，
	// 绝不返回本地拼装的假 code_url（2026-09-16 整改口径，#41 实装协议后继续保留）
	...
	if _, err := p2.CreateOrder(&PayRequest{OrderNo: "RO1", Amount: 100}); err == nil {
		t.Fatal("微信资质不全时应 fail-closed 报错（禁止假二维码出单）")
	}
...
// alipay 未配置资质应报错；资质不全（缺回调地址）同样 fail-closed 拒绝出单；
// 回调 RSA2 验签独立可用。真实下单协议的完整正/反例见 gateway_sdk_test.go。
func TestAlipayProvider(t *testing.T) {
	...
	// 有 AppID/私钥/公钥但缺回调地址（NotifyURL 与 NotifyBase 皆空）：仍须拒绝出单，
	// 不再返回 alipay:// 占位串（2026-09-16 整改口径，#41 实装协议后继续保留）
	if _, err := p2.CreateOrder(&PayRequest{OrderNo: "RO1", Amount: 100}); err == nil {
		t.Fatal("支付宝资质不全时应 fail-closed 报错（禁止占位收款码出单）")
	}
	// 构造已签名的表单回调

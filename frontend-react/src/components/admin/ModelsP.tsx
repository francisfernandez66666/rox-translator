...
import {
  ...
  adminPolicy, adminPolicySave, type Any,
} from '@/api'
...
export function ModelsP() {
  ...
  return (
    <>
      ...
      <Panel title={t('models.routingTitle')}>
        <div style={{ fontSize: 13, color: 'var(--adm-hint)', marginBottom: 8 }}>{t('models.onlineHint')}</div>
        ...
        <p style={{ fontSize: 13, color: 'var(--adm-hint)', margin: '8px 0 0' }}>
      ...
      <Panel title={t('models.llmKeyTitle')}>
        <div style={{ fontSize: 13, color: 'var(--adm-hint)', marginBottom: 8 }}>{t('models.llmKeyHint')}</div>
        <div style={rowMt}>
          {/* 翻译主密钥状态回显：后端只下发 configured 布尔（✓/✗ 图形字符已于 2026-09-18 移除，
              状态改由「已配置/未配置」文案表意），明文密钥任何情况下都不回传前端。 */}
          <span style={{ fontSize: 14 }}>{t('models.translationKeyLabel')}：{keyState.translation ? ` ${t('models.configured')}` : ` ${t('models.notConfigured')}`}</span>
 ...
 {keyState.embedding && <span style={{ fontSize: 13, color:'var(--adm-ok-tx)'}}> {keyState.embeddingMasked}</span>}
        </div>
      </Panel>
      ...
      {stageCards.map((st) => (
        <Panel key={st.key} title={st.title}>
          <div style={{ fontSize: 13, color: 'var(--adm-hint)', marginBottom: 8 }}>{st.hint}</div>
          <div style={rowMt}>
 ...
 {stActive(st.key) && <span style={{ fontSize: 13, color:'var(--adm-ok-tx)'}}> {t('models.stageConfigured'as never)}</span>}
          </div>
      ...
      <div style={rowMt}>
        ...
        <span style={{ fontSize: 13, color: 'var(--adm-hint)' }}>{stageHint ? tpl('models.stageActive', { count: stageHint }) : t('models.stageNone')}</span>
      </div>

...
// 职责：合并「邮件模板 / 流程引擎 / 系统告警」三个子面板为单一「系统设置」tab，
...
import { AlertsP } from './panels_a'
import { WorkflowP } from './panels_d'

/** 系统设置面板组件（超管 L4）：邮件模板 / 流程引擎 / 系统告警 三合一 */
export default function SystemSettingsP() {
  const [, t] = useT()
  const [tab, setTab] = useState<'mailTpl' | 'workflow' | 'alerts'>('mailTpl')

  return (
    <Tabs value={tab} onChange={(v) => setTab(v as 'mailTpl' | 'workflow' | 'alerts')}>

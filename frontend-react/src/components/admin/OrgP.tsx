...
export function OrgP() {
 ...
 const orgIcon = (o: OrgInfo): string => (o.type ==='dept'?'': o.type ==='org'?'':'')
  ...
  return (
    ...
    <Tabs activeKey={tab} onChange={(k) => setTab(k as 'org' | 'invite')} items={[
      ...
      <p style={{ fontSize: 13, color: 'var(--adm-faint)', margin: '0 0 12px' }}>{t('org.treeHint')}</p>
              ...
              <span style={{ display: 'flex', alignItems: 'center', gap: 4, flexWrap: 'wrap' }}>
                {o.type !== 'root' && myLevel >= 3 && (
                  <span className={isOverBudget(o) ? 'budget-over' : ''}
                        ...
                        style={{ cursor: 'pointer', fontSize: 13, color: isOverBudget(o) ? 'var(--lc-danger)' : 'var(--lc-text-3)', border: `1.2px solid ${isOverBudget(o) ? 'var(--lc-danger)' : 'var(--lc-border-card)'}`, borderRadius: 8, padding: '0 6px' }}>
                     {budgetText(o)}
                  </span>
                )}
                {o.type !== 'root' && myLevel >= 3 && (
 ...
 <Button size="sm" variant="danger" title={t('org.delete')} aria-label={t('org.delete')} onClick={(e) => { e.stopPropagation(); deleteOrg(o) }}><Icon n="trash" /></Button>
        ...
        <div style={{ flex: '2 1 480px', minWidth: 360 }}>
          ...
          <p style={{ fontSize: 13, color: 'var(--adm-faint)', margin: '0 0 12px' }}>{t('org.usersHint')}</p>
          {/* 数据表格 */}
          ...
          {!orgUserList.length && <div style={{ fontSize: 13, color: 'var(--adm-faint)', padding: 8 }}>{t('org.noUsers')}</div>}
          ...
          <div style={{ marginTop: 16, border: '1.2px solid var(--adm-line)', borderRadius: 8, padding: 14 }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
              <span style={{ fontSize: 13, color: 'var(--adm-hint)' }}>{t('org.orgLabel')}</span>
              ...
              <span style={{ fontSize: 13, color: 'var(--adm-hint)' }}>{t('org.roleLabel')}</span>
              ...
              <span style={{ fontSize: 13, color: 'var(--adm-faint)', flex: 1, minWidth: 120 }}>{t('org.cascadeHint')}</span>
          ...
          <Dialog open={!!importResult} onCancel={() => setImportResult(null)}
            ...
            <DataTable<any> rowKey={(row) => String(row.username)} rows={importResult || []}
                   columns={[
                     ...
                     { key: 'message', title: '', render: (row) => <span style={{ fontSize: 13, color: row.ok ? 'var(--lc-text-3)' : 'var(--lc-danger)' }}>{row.message}</span> },
                  ]} />
          </Dialog>
      ...
      <Dialog open={!!budgetModal} onCancel={() => setBudgetModal(null)}
        ...
        {budgetModal && (
          <>
            <p style={{ fontSize: 13, color: 'var(--adm-faint)', margin: '0 0 10px' }}>{tpl('org.budgetHint', { used: fmtPoints(budgetModal.used) })}</p>
            <label style={{ display: 'block', marginBottom: 4, fontSize: 13, color: 'var(--adm-hint)' }}>{t('org.budgetLimit')}</label>
 ...
 title={`${t('org.inviteTitle')} · ${inviteModal?.name || ''}`}>
        {inviteModal && (
          <>
            <p style={{ fontSize: 13, color: 'var(--adm-faint)', margin: '0 0 10px' }}>{t('org.inviteHint')}</p>
 ...
 <Button variant="primary"disabled={!inviteCodeInput.trim()} onClick={createInvite}> {t('org.inviteCreate')}</Button>
            ...
            ) : <p style={{ fontSize: 13, color: 'var(--adm-faint)' }}>{t('org.inviteNoCodes')}</p>}
          </>
        )}
      </Dialog>

...
export default function BrandP() {
  ...
  return (
    <Panel title={t('brand.title')}>
      ...
      {tab === 'brand' && (
        <>
          <p style={{ fontSize: 14, color: 'var(--adm-hint)', marginBottom: 12 }}>{t('brand.hint')}</p>
      <div style={{ fontSize: 14, color: 'var(--adm-info-tx)', background: 'var(--adm-info-bg)', border: '1.2px solid var(--adm-info-bd)', borderRadius: 8, padding: '10px 12px', marginBottom: 12, lineHeight: 1.6 }}>
         {t('brand.featureDedicated')}
      </div>
      {/* 超管：租户选择器 */}
      {isSuper && (
        <div style={{ marginBottom: 12 }}>
          <div style={{ fontSize: 14, marginBottom: 4 }}>{t('brand.tenantSelect')}</div>
      ...
      {isSuper && targetTenantId > 1 && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'var(--adm-purp-bg)', border: '1.2px solid var(--adm-purp-bd)', borderRadius: 8, padding: '10px 12px', marginBottom: 12, fontSize: 14 }}>
      ...
      {/* 未获得编辑权限时显示锁定提示 */}
      {!editable && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--adm-warn-bg)', border: '1.2px solid var(--adm-warn-bd)', color: 'var(--adm-warn-tx)', borderRadius: 8, padding: '10px 12px', marginBottom: 12, fontSize: 14 }}>
      ...
      {!loaded ? (
        ...
        <div style={{ maxWidth: 560, display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div>
            <div style={{ fontSize: 14, marginBottom: 4 }}>{t('brand.nameEn')}</div>
           ...
           <div>
             ...
             <div style={{ fontSize: 14, margin: '8px 0 4px' }}>{t('brand.logoUrl')}</div>
           ...
           <div>
             <div style={{ fontSize: 14, marginBottom: 4 }}>{t('brand.domain')}</div>
              ...
              <div style={{ fontSize: 13, color: 'var(--adm-faint)', marginTop: 4 }}>
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, alignItems: 'center', marginTop: 8 }}>
                ...
                {loginLayout.mode === 'split' && (
                  <>
                    <div style={{ fontSize: 14 }}>{t('brand.loginSide')}</div>
              ...
              {homeBg && (
                <div style={{ marginTop: 8 }}>
                  {loginLayout.mode === 'split' ? (
                        ...
                        <>
                          ...
                          <div ref={splitFormRef} style={{ flex: 1, position:'relative', background:'rgba(231,233,234,0.06)', overflow:'hidden'}}>
                            <div onMouseDown={(e) => { e.preventDefault(); cardDragRef.current = { startX: e.clientX, startY: e.clientY, x0: loginCardPos.x, y0: loginCardPos.y, el: splitFormRef.current } }}
                              style={{ position:'absolute', left: `${loginCardPos.x}%`, top: `${loginCardPos.y}%`, transform:'translate(-50%,-50%)', width: 120, height: 80, background:'#0E1014', borderRadius: 8, boxShadow:'0 6px 20px rgba(0,0,0,.2)', display:'flex', alignItems:'center', justifyContent:'center', fontSize: 12, color:'#889', cursor:'move'}}>
                              {t('brand.cardPreview')}
                            </div>
                          </div>
                          {/* 右侧：背景图展示 */}
                    ...
                    >
                      ...
                      <div
                        ...
                        style={{ position:'absolute', left: `${loginCardPos.x}%`, top: `${loginCardPos.y}%`, transform:'translate(-50%,-50%)', width: 140, height: 90, background:'#0E1014', borderRadius: 8, boxShadow:'0 6px 20px rgba(0,0,0,.25)', display:'flex', alignItems:'center', justifyContent:'center', fontSize: 12, color:'#889', cursor:'move'}}
                      >
                        {t('brand.cardPreview')}
                      </div>
                      {/* 背景图拖拽提示文字 */}
                      <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#cdd', fontSize: 13, pointerEvents: 'none' }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, alignItems: 'center', marginTop: 10, maxWidth: 420 }}>
                    <div style={{ fontSize: 14 }}>{t('brand.homeBgMode')}</div>
                    ...
                    <span style={{ fontSize: 13, color: 'var(--adm-faint)', minWidth: 40 }}>{homeBgStyle.scale.toFixed(1)}x</span>
              ...
              <div style={{ fontSize: 14, margin: '8px 0 4px' }}>{t('brand.homeBgUrl')}</div>
              <input className="lc-input" value={homeBg} disabled={!editable} onChange={(e) => setHomeBg(e.target.value)} placeholder="https://…/bg.png" />
              <div style={{ fontSize: 13, color: 'var(--adm-faint)', marginTop: 4 }}>{t('brand.homeBgHint')}</div>
            </div>

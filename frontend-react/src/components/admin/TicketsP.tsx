...
export function TicketsP() {
  ...
  return (
    <>
      ...
      <p style={{ fontSize: 14, color: 'var(--adm-hint)', margin: '0 0 12px' }}>{isSuper ? t('fb.superHint') : t('fb.userHint')}</p>
      ...
      {selected && (
        <div style={cardStyle}>
          ...
          {selected.with_context && (
            <div style={{ background: 'var(--adm-soft)', border: '1px dashed var(--adm-line)', borderRadius: 8, padding: '8px 10px', marginTop: 8, fontSize: 13.5 }}>
          ...
          {/* 往来回复：底色按角色区分、顺序完全照后端返回排，前端不再二次排序以免两端时间线不一致
              （两处气泡底色仍是硬编码浅色，按工程约定后续应收敛为 --adm-* 语义令牌以适配暗色） */}
          {selected.replies && selected.replies.length ? (
            <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 6 }}>
              {selected.replies.map((r: Any, i: number) => (
                <div key={i} style={{ background: r.role === 'admin' ? '#e8f0fe' : '#f5f6f8', borderRadius: 8, padding: '6px 10px', fontSize: 14 }}>
                  <div style={{ fontSize: 12, color: 'var(--adm-faint)', marginBottom: 2 }}>{r.name} · {r.role === 'admin' ? t('tickets.roleAdmin') : t('tickets.roleUser')} · {fmtAt(r.at)}</div>
                  <div style={{ whiteSpace: 'pre-wrap' }}>{r.content}</div>
                </div>
              ))}
            </div>
          ) : <div style={{ fontSize: 13, color: 'var(--adm-faint)', marginTop: 8 }}>{t('fb.noReplies')}</div>}
 ...
 {isSuper && <Button variant="primary"onClick={() => void doResolve()}> {t('fb.complete')}</Button>}
...
) : <div style={{ fontSize: 13, color: 'var(--adm-ok-tx)', marginTop: 8 }}> {t('fb.archivedHint')}</div>}
        </div>
      )}
      ...
      {!selected && fbTab !== 'review' && (
        <>
          ...
          {!isSuper && (
            <Panel title={t('fb.submitTitle')}>
              ...
              <div style={{ ...rowMt, marginTop: 8 }}>
                <span style={{ flex: 1, fontSize: 13, color: 'var(--adm-faint)' }}>{newContent.length}/1000</span>
          ...
          <div style={rowMt}>
            ...
            <span style={{ fontSize: 13, color: 'var(--adm-faint)' }}>{tpl('fb.count', { n: feedbacks.length })}</span>
          </div>
          {/* 数据表格 */}
      ...
      {approvalTickets.map((tk) => (
        <div key={tk.id} style={{ ...cardStyle, marginTop: 12 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
            ...
            <span style={{ fontSize: 13, color: 'var(--adm-faint)' }}>{tk.status}</span>
          </div>
          <p style={{ fontSize: 14, color: 'var(--adm-hint)', margin: '0 0 8px' }}>{tk.source_text}</p>
      ...
      {!approvalTickets.length && <div style={{ fontSize: 14, color: 'var(--adm-faint)' }}>{t('tickets.noApproval')}</div>}
      ...
      <Dialog open={!!approveDlg} onCancel={() => setApproveDlg(null)}
        ...
        {approveDlg && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <div style={{ fontSize: 14, color: 'var(--adm-hint)' }}>{String(approveDlg.row.source_text)}</div>

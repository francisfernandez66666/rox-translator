    ...
    >
      <div className="lc-dialog" role="dialog" aria-modal="true" style={{ width: 560 }}>
        ...
        <div className="lc-dialog__body">
          <div style={{ fontSize: 15, color: 'var(--lc-text-3)', marginBottom: 12 }}>{t('kb.topbarHint')}</div>
      ...
      {file && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 15, color: 'var(--lc-text-2)', marginBottom: 12 }}>
      ...
      {recognized && (
        <div style={{ fontSize: 15, marginBottom: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginTop: 10 }}>
            {options.length === 0 ? (
                  <span style={{ fontSize: 15, color: 'var(--lc-text-3)' }}>{t('kb.noPkg')}</span>
            ) : (
                  <>
                    <select
                      className="lc-select"
            ...
            {pkgId > 0 ? (() => {
              ...
              return p ? (
                    <span style={{ fontSize: 14, color: 'var(--lc-text-3)' }}>
      ...
      {result && (
            <div style={{ fontSize: 15, color: result.success ? 'var(--lc-text)' : 'var(--lc-danger)' }}>
          {result.message || (result.success ? t('kb.import') + ' OK' : t('kb.importErr').replace('{msg}', ''))}
            </div>
          )}
        </div>
        <div className="lc-dialog__actions">
          <button type="button" className="lc-btn lc-btn--secondary" style={{ padding: '8px 16px', fontSize: 15 }} onClick={onClose}>{t('common.close')}</button>
        </div>
      </div>
    </div>,
    document.body,
